...
public class OstacoloAdiacenteDestraCondizione implements ICondizione, IStrategiaComponent, Serializable {
    ...
    public boolean eseguiti(ITank tankTurno, ITank tankAvversario, CampoBattaglia campo) {
        ...
        OrientamentoEnum orientamentoDestra = OrientamentoEnum.getDestra(orientamentoTankTurno);
        List campoDirezione = campo.getCaselleByOrientamento(casellaTankTurno,orientamentoDestra);
        ...
        if (verifica) {
            System.out.println("OSTACOLO DESTRA");
        } else {
            System.out.println("OSTACOLO NON DESTRA");
        }
        if (!this.vera) {
            verifica = !verifica;
        }
